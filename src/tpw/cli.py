...
def build(date):
 ...
 try:
  ...
  (site/'.nojekyll').write_text('');(site/'assets/css/app.css').write_text(css(),encoding='utf-8');(site/'assets/js/app.js').write_text(js(),encoding='utf-8')
  build_site(aggs,date,site,status,series=series,scores=scores,seasonality=season,advice=advice,traceability=trace,quality=quality)
...
def verify_site(root=ROOT/'site',date=None):
 ...
 for p in root.rglob('*'):
  if p.is_file():
   total+=p.stat().st_size;largest=max(largest,(p.stat().st_size,p))
...
def validate_data(date):
 load_date(date)
 required=(ROOT/'data/aggregates/daily'/date[:4]/date[5:7]/(date+'.json'),ROOT/'data/seasonality'/(date[:7]+'.json'),ROOT/'data/traceability/current.json',ROOT/'data/advice'/date[:4]/date[5:7]/(date+'.json'),ROOT/'data/quality'/date[:4]/date[5:7]/(date+'.json'))
