...
from .seasonality import CATEGORIES as AFA_CATEGORIES,build_catalog,catalog_from_seasonality,fetch_official,load_manual,map_catalog,seasonality_refresh_decision,with_source_status
...
from .categories import load_category_registry
from .produce_icons import read_produce_icon_sprite, resolve_produce_icon, validate_produce_icon_sprite
...
from .season_extensions import load_extension_catalog, merge_season_catalog
...
def ingest_sources(source_batches,start,end):
 ...
 # Deliberately NOT threading load_category_registry(ROOT) here (unlike validate-config/build/
 # verify-site/validate-data): several existing 8066-ingest tests outside this work order's file
 # ownership (tests/contract/test_source_adapter*.py) drive this through a minimal isolated ROOT
 # copy that carries only config/produce.yml, not config/produce-categories.json. The default
 # registry is safe here because config/produce.yml never configures anything outside
 # fruit/vegetable, so canonical_map's watchlist check is identical either way.
...
def seasonality_rows(month):
 ...
 return rows
def seasonality_catalog(month,season=None,registry=None):
 ...
 if not rows or any(row.get('month')!=month or row.get('category') not in registered for row in rows):raise ValueError('seasonality catalog is empty or mismatched')
 return rows
def merged_season_catalog(month,registry,season=None):
 return merge_season_catalog(seasonality_catalog(month,season,registry),load_extension_catalog(ROOT,month,registry))
def persist_seasonality(month,opener=None,fetched_at=None):
 ...
 try:
  ...
  if previous_catalog and all(row.get('month')==month and row.get('source_status') in ('live','stale') for row in previous_catalog):
   previous_counts={category:sum(row.get('category')==category for row in previous_catalog) for category in AFA_CATEGORIES}
   current_counts={category:sum(row.get('category')==category for row in catalog) for category in AFA_CATEGORIES}
  ...
  if catalog_lkg:catalog_lkg={row.get('category') for row in previous_catalog}==set(AFA_CATEGORIES)
...
def build(date):
 ...
 try:
  ...
  series=build_series(all_aggs,date);season=seasonality_rows(context_month);season_map_config=load_season_map_config(ROOT);season_catalog=merged_season_catalog(context_month,season_map_config.category_registry,season);season_map_payload=build_season_map_payload(ROOT,season_catalog,date);scores=score_all(series,season);advice=generate_advice(scores,date);trace,trace_profile=traceability_context(publication['requested_date']);trace_events,trace_event_profile=traceability_event_context(publication['requested_date'])
  warnings=[]
  ...
  build_site(aggs,date,site,status,series=series,scores=scores,seasonality=season,season_catalog=season_catalog,advice=advice,traceability=trace,traceability_status=trace_profile,traceability_events=trace_events,traceability_event_status=trace_event_profile,quality=quality,publication_status=publication,season_map_payload=season_map_payload,county_svg=season_map_config.svg,category_registry=season_map_config.category_registry)
  reports=stage/'reports'
  if (ROOT/'reports').exists():shutil.copytree(ROOT/'reports',reports)
  rp=reports/'daily'/date[:4]/date[5:7];rp.mkdir(parents=True,exist_ok=True);rp.joinpath(date+'.md').write_text(render_report(aggs,scores,advice,quality,date,season_map_config.category_registry),encoding='utf-8')
...
def verify_site(root=None,date=None):
 ...
 if current_path.exists():
  ...
  if public_current.get('prototype_complete'):
   ...
   if registry.no_official():
    if map_html.count('data-season-semantics-unknown')!=22:raise ValueError('season map is missing the per-county no-official-season-registry unknown line')
    if 'data-season-semantics-notice' not in map_html:raise ValueError('season map intro is missing the no-official-season-registry notice')
   elif 'data-season-semantics-unknown' in map_html or 'data-season-semantics-notice' in map_html:raise ValueError('season map declares no-official-season-registry notices without any such category')
 for p in files:
  text=p.read_text();
  ...
  for reference in parser.uses:
   ...
   if fragment not in validate_produce_icon_sprite(target.read_bytes(),registry):raise ValueError('unknown SVG sprite fragment '+reference+' in '+str(p))
 if date:
  ...
  if current.get('prototype_complete'):
   ...
   catalog_categories={row['category'] for row in catalog}
   for extra_category in registry.categories:
    if extra_category.market_watchlist:continue
    button_token="data-filter='"+extra_category.id+"'"
    if extra_category.id in catalog_categories:
     if button_token not in season_html:raise ValueError('season page is missing a filter button for a category with rows: '+extra_category.id)
    elif button_token in season_html:raise ValueError('season page has a filter button for a category with no rows: '+extra_category.id)
   if registry.no_official():
    if 'data-season-semantics' not in season_html:raise ValueError('season page is missing the season-semantics section')
   elif 'data-season-semantics' in season_html:raise ValueError('season page declares a season-semantics section without any no-official-season-registry category')
...
def main(argv=None):
 ...
 if a.cmd=='validate-config':
  items=config();map_config=load_season_map_config(ROOT);registry=map_config.category_registry;canonical_map(items,registry);assert sum(x['category']=='fruit' and x.get('enabled') for x in items)>=10 and sum(x['category']=='vegetable' and x.get('enabled') for x in items)>=10;validate_produce_icon_sprite(read_produce_icon_sprite(registry=registry),registry);print('config valid: 20 mapped items, '+str(len(map_config.county_registry['counties']))+' counties, '+str(len(map_config.market_registry['markets']))+' verified markets, '+str(len(registry.ids()))+' produce categories')
